            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.length === 0 ? (
                ...
                <tr key={offer.id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-6 py-4">
                    <span className="bg-red-100 text-red-700 text-xs font-bold px-2.5 py-1 rounded-full">{offer.discount_percent}% OFF</span>
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-500">
                    {offer.valid_until ? new Date(offer.valid_until).toLocaleDateString() : 'No expiry'}
                  </td>
                  <td className="px-6 py-4">
                    <button onClick={() => toggleActive(offer)} className={`px-2.5 py-1 rounded-full text-xs font-semibold transition ${offer.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
