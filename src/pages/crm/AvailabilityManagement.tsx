...
export default function AvailabilityManagement() {
 ...
 className="flex items-center gap-2 border border-gray-300 text-gray-700 px-4 py-2.5 font-semibold hover:bg-gray-50 transition text-sm">
 ...
 <span className="flex items-center gap-1"><span className="w-3 h-3 bg-green-400 inline-block"></span>Available</span>
 <span className="flex items-center gap-1"><span className="w-3 h-3 bg-orange-400 inline-block"></span>Limited (&le;3)</span>
 <span className="flex items-center gap-1"><span className="w-3 h-3 bg-red-400 inline-block"></span>Full/Blocked</span>
 </div>
 </div>

 {loading ? (
 <div className="flex items-center justify-center py-16"><div className="animate-spin h-8 w-8 border-b-2 border-red-600" /></div>
 ) : (
 ...
 <div className="flex items-center gap-2">
 <div className={`w-2 h-2 flex-shrink-0 ${slot.is_blocked || seatsLeft === 0 ? 'bg-red-400' : seatsLeft <= 3 ? 'bg-orange-400' : 'bg-green-400'}`} />
 ...
 className={`px-2.5 py-1 text-xs font-semibold transition ${slot.is_blocked ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
 ...
 <div className="flex gap-3 pt-2">
 <button type="submit" disabled={bulkSaving} className="flex items-center gap-2 bg-red-600 text-white px-6 py-2.5 font-semibold hover:bg-red-700 transition disabled:opacity-60 text-sm">
 <Check className="h-4 w-4" />{bulkSaving ? 'Creating...' : 'Create Slots'}
 </button>
 <button type="button" onClick={() => setBulkModal(false)} className="px-6 py-2.5 border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-50 transition">Cancel</button>
 </div>
 </form>
 </div>
 </div>
