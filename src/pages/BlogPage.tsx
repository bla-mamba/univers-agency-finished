...
export default function BlogPage() {
  ...
  const filtered = posts.filter((p) => {
    ...
    const matchCategory = activeCategory === 'Të gjitha' || p.category === activeCategory;
      ...
      <section className="relative h-[420px] overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-8 h-full flex flex-col justify-end pb-14">
          ...
          <p className="text-white/70 max-w-xl leading-relaxed text-base font-light">
            Analiza të detajuara mbi destinacionet, këshilla praktike për udhëtimin dhe
            përmbledhje nga terreni, të përgatitura nga konsulentët tanë dhe specialistët e
            destinacioneve.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-12">
        ...
        {!loading && (
          <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-200">
            <p className="text-sm text-gray-500">
              {filtered.length === 0
                ? 'Asnjë artikull nuk përputhet me filtrat tuaj'
                : `${filtered.length} ${
                    filtered.length === 1 ? 'artikull' : 'artikuj'
                  }${activeCategory !== 'Të gjitha' ? ` në kategorinë ${activeCategory}` : ''}`}
            </p>

            {(search || activeCategory !== 'Të gjitha') && (
              <button
                onClick={() => {
                  setSearch('');
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white border border-gray-200">
            ...
            <h3 className="text-base font-bold text-gray-800 mb-1">Nuk u gjet asnjë artikull</h3>
            <p className="text-sm text-gray-400 max-w-sm mx-auto">
              {posts.length === 0
                ? 'Ekipi ynë editorial po përgatit përmbajtje të re. Ju ftojmë të riktheheni së shpejti.'
                : 'Provoni të ndryshoni fjalët e kërkimit ose të zgjidhni një kategori tjetër.'}
            </p>
          </div>
        ) : (
          <>
                        ...
                        <div className="flex flex-col gap-1 text-xs text-gray-400">
                          ...
                          {featured.published_at && (
                            <span className="flex items-center gap-1.5">
                              ...
                              {new Date(featured.published_at).toLocaleDateString('sq-AL', {
                                day: 'numeric',
                                month: 'long',
                                year: 'numeric',
                              })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 items-start">
            <div className="md:col-span-2">
              ...
              <h2 className="text-3xl font-bold text-gray-900 mb-5 leading-tight tracking-tight">
                Si përgatitet përmbajtja jonë
              </h2>
              <p className="text-gray-500 text-sm leading-relaxed mb-4">
                Të gjithë artikujt e publikuar në këtë revistë hulumtohen, shkruhen dhe
                rishikohen nga konsulentët tanë të udhëtimit dhe specialistët e destinacioneve.
                Përmbajtja mbështetet në përvojë të drejtpërdrejtë nga terreni, burime të
                verifikuara dhe bashkëpunim me partnerë lokalë. Nuk publikojmë përmbajtje të
                sponsorizuar pa e deklaruar qartë.
              </p>
              <p className="text-gray-500 text-sm leading-relaxed">
                Kushtet në destinacion, kërkesat e hyrjes dhe detajet operative mund të
                ndryshojnë shpesh. Edhe pse përmbajtja jonë përditësohet rregullisht, u
                rekomandojmë lexuesve që informacionin e ndjeshëm ndaj kohës ta verifikojnë
                drejtpërdrejt me ambasadën, linjën ajrore ose ofruesin përkatës të shërbimit
                përpara udhëtimit.
              </p>
            </div>
            ...
            <div className="border border-gray-200">
              {[
                {
                  label: 'Shkruar nga',
                  value: 'Konsulentë udhëtimesh me përvojë të drejtpërdrejtë nga terreni',
                },
                {
                  label: 'Rishikuar nga',
                  value: 'Specialistë destinacionesh dhe menaxherë operacionesh',
                },
                {
                  label: 'Frekuenca e përditësimit',
                  value: 'Përmbajtja rishikohet dhe përditësohet vazhdimisht',
                },
                {
                  label: 'Përmbajtje e sponsorizuar',
                  value: 'Deklarohet qartë. Pjesa më e madhe e përmbajtjes është e pavarur.',
                },
              ].map(({ label, value }, i, arr) => (
                <div
                  key={label}
                  className={`p-5 ${i < arr.length - 1 ? 'border-b border-gray-200' : ''}`}
                >
